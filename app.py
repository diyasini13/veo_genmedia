...
os.environ['GOOGLE_APPLICATION_CREDENTIALS'] = 'path-to-serice-account-key.json-file'
...
#Generate image using imagen3 or imagen4 based on model_name
...
def generate_video_with_veo(input_type: str, input_content: bytes | str, video_prompt: str) -> bytes | None:
    """
    Generates a video using the Veo model (veo-2.0-generate-001) on Vertex AI.
...
# New state for Veo model choice in Image to Long Video tab - REMOVED!
# if 'img_long_video_veo_model_choice' not in st.session_state:
#     st.session_state.img_long_video_veo_model_choice = "Veo2" # Default to Veo2
...
with tab_image_to_video:
    ...
    if st.session_state.image_bytes:
        st.image(st.session_state.image_bytes, caption=f"Generated by {st.session_state.imagen_model_choice}", use_container_width=True)
    else:
...
with tab_text_to_video:
    st.header("Text to Video Generation with Veo2")
